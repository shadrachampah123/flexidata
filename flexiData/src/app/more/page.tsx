...
export default async function MorePage() {
  const [wallet, alerts] = await Promise.all([getWallet(), getActiveAlerts()]);
  ...
  return (
    <div>
      ...
      {/* Profile card */}
      <div className="animate-fade-up flex items-center gap-4 rounded-[1.75rem] border border-black/[0.05] bg-paper p-5 shadow-sm dark:border-line dark:bg-card">
      ...
      {/* Support */}
      <div className="animate-fade-up mt-5 rounded-[1.75rem] border border-black/[0.05] bg-paper p-5 shadow-sm dark:border-line dark:bg-card" style={{ animationDelay: "140ms" }}>
      ...
      <p className="mt-8 text-center text-[10px] font-semibold tracking-wide text-zinc-400 dark:text-zinc-600">
        {APP_NAME} v1.0 • Made for Ghana • Simulated environment
      </p>
    </div>
  );
}
